    ...
    <div className="w-screen h-screen bg-black relative overflow-hidden font-mono select-none">
      ...
      {gameState === 'menu' && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-10 pointer-events-auto px-4">
          <h1 className="text-4xl md:text-6xl font-black text-cyan-400 mb-4 md:mb-8 drop-shadow-[0_0_20px_rgba(34,211,238,0.8)] tracking-tighter text-center">
            NEON ARENA
          </h1>
          <p className="text-gray-400 mb-8 text-center max-w-md text-sm md:text-base">
            {isMobile ? (
              <>Use left joystick to move, right joystick to look. Tap center button to shoot.<br/>Hit enemies! Don't get hit!</>
            ) : (
              <>WASD to move. Mouse to look and click to shoot.<br/>Hit enemies for points. Don't get hit!</>
            )}
          </p>
      ...
      {gameState === 'gameover' && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-10 pointer-events-auto px-4">
          <h1 className="text-4xl md:text-6xl font-black text-red-500 mb-4 drop-shadow-[0_0_20px_rgba(239,68,68,0.8)] tracking-tighter text-center">
            GAME OVER
          </h1>
          <div className="text-xl md:text-3xl text-cyan-400 mb-8 font-bold">
            FINAL SCORE: {score}
          </div>
          <div className="w-full max-w-[320px]">
            <button
              id="start-button"
              onClick={() => startGame()}
              className="w-full px-8 py-4 bg-cyan-500/20 border-2 border-cyan-400 text-cyan-400 text-xl font-bold rounded hover:bg-cyan-400 hover:text-black transition-all duration-200"
            >
              PLAY AGAIN
            </button>
          </div>
